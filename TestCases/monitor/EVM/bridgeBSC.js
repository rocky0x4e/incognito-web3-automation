...
const logger = getLogger("BNB_BSC")
...
describe(`[ ======  BSC BRIDGE - SHIELD ======  ]`, async () => {
    const tokenID = ENV.Testbed.Tokens.BNB_BSC
    const tokenUnifiedID = ENV.Testbed.Tokens.BNB_UT
    ...
    let slack = makeSlackAlert("EVM_BSC_Shielding")
...
describe(`[======  BSC BRIDGE -- UNSHIELDING ====== ]`, async () => {
    const tokenID = ENV.Testbed.Tokens.BNB_BSC
    const tokenUnifiedID = ENV.Testbed.Tokens.BNB_UT
    let web3 = await new Web3(new Web3.providers.HttpProvider(ENV.Testbed.BSCFullnode.url))
    ...
    let extAccount = ACCOUNTS.Evm.get(0).setProvider(ENV.Testbed.BSCFullnode.url)
    let backendApi = new BackendApi()
    let slack = makeSlackAlert("EVM_BSC_UnShielding")
