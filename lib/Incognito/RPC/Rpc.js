...
class IncRpc extends BaseRpc {
    ...
    async createRawTx(senderPrivateK, recevierPaymentK, amount, fee = 10, privacy = 1) {
        ...
        let res = await this.withMethod("createtransaction").withParams(
            [senderPrivateK, receive, fee, privacy]).execute()
        return res.result
    }
    ...
    async pdexv3_getTradeStatus(tx) {
        return await this.withMethod("pdexv3_getTradeStatus").withParams([tx]).execute()
    }
    ...
    async pdexv3_getAddOrderStatus(tx) {
        return await this.withMethod("pdexv3_getAddOrderStatus").withParams([tx]).execute()
    }

    async pdexv3_getWithdrawOrderStatus(tx) {
        return await this.withMethod("pdexv3_getWithdrawOrderStatus").withParams([tx]).execute()
    }

    async pdexv3_getContributionStatus(tx) {
        return await this.withMethod("pdexv3_getContributionStatus").withParams([tx]).execute()
    }

    async pdexv3_getWithdrawLiquidityStatus(tx) {
        return await this.withMethod("pdexv3_getWithdrawLiquidityStatus").withParams([tx]).execute()
    }

    async pdexv3_getWithdrawalLPFeeStatus(tx) {
        return await this.withMethod("pdexv3_getWithdrawalLPFeeStatus").withParams([{ "ReqTxID": tx }]).execute()
    }
}
