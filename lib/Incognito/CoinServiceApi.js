...
class CoinServiceApi extends BaseApi {
    constructor(obj = ENV.Testbed.Incognito.CoinService) {
        super(obj)
    }
    ...
    async getTokenContract(tokenID, network = null) {
        ...
        for (const token of tokenMatch.ListUnifiedToken) {
            let isFind = true
            for (const item of network.toLowerCase().split("")) {
                if (!(token.Network).toLowerCase().includes(item)) {
                    isFind = false
                    break
                }
            }
            if (isFind) {
                var childTokenMatch = token
                break;
            }
        }
        return typeof childTokenMatch == "undefined" ? null : childTokenMatch.ContractID;
    }

    async getChildTokenInfo(tokenID, network = null) {
        await this.initListToken();
        logger.debug("Finding token contract by ID")
        var tokenMatch = BaseApi.LIST_TOKEN.find((element) => element.TokenID == tokenID);
        if (typeof tokenMatch == "undefined") {
        ...
        return typeof childTokenMatch == "undefined" ? null : childTokenMatch;
    }

    async getTokenIdFromSymbol(symbol) {
        await this.initListToken();
        for (const token of BaseApi.LIST_TOKEN) {
            if (token.Symbol.toLowerCase() == symbol.toLowerCase() && token.Verified == true) {
    ...
    async gettxstatus({ tx }) {
        let path = `txservice/gettxstatus?txhash=${tx}`;
        return this.get(path);
    }
}
