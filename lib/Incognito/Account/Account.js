...
class IncAccount {
    ...
    async waitForNewShieldRecord({ tokenId,
        ...
        while (timedOut > 0) {
            ...
            logger.debug(`resAfter.data.Result.length =  ${resAfter.data.Result.length}  ---- resBefore.data.Result.length = ${resBefore.data.Result.length}`)
            logger.debug('[ %d ] Waiting backend listen tx shield .... ', timedOut)
