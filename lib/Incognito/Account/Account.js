...
class IncAccount {
    ...
    async waitForNewShieldRecord({ tokenId,
        ...
        let id = 0
        const resBefore = await backendApi.historyByTokenAccount(this.paymentK, tokenId, SignPublicKeyEncode)

        while (timedOut > 0) {
            let resAfter = await backendApi.historyByTokenAccount(this.paymentK, tokenId, SignPublicKeyEncode)
