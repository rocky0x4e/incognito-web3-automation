...
class IncAccount {
    ...
    async waitForNewShieldRecord({ tokenId,
        ...
        let id = 0
        const resBefore = await backendApi.historyByTokenAccount({
            WalletAddress: this.paymentK,
            PrivacyTokenAddress : tokenId
        })

        while (timedOut > 0) {
            let resAfter = await backendApi.historyByTokenAccount({
                WalletAddress: this.paymentK,
                PrivacyTokenAddress: tokenId
            })
            logger.info(`resAfter.data.Result.length =  ${resAfter.data.Result.length}  ---- resBefore.data.Result.length = ${resBefore.data.Result.length}`)
            logger.info(`[${timedOut}] Waiting backend listen tx shield .... `)
...
class IncAccountGroup {
    ...
    importKeyList(keyList, node = null) {
     ...
     * @returns {IncAccount} IncAccount object: if all = false
     * @returns {Array<IncAccount>}: Array of IncAccount if all = true
    */
