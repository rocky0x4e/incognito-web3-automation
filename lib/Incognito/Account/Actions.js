...
const loggerCli = getLogger("CliAction")
const loggerRpc = getLogger("RpcAction")
const loggerSdk = getLogger("SdkAction")
...
class BaseAction {
    ...
    _fromMe() {
        return this.keyInfo.PrivateKey.slice(-8)
    }
}

class CliAction extends BaseAction {
    async getBalanceAll() {
        loggerCli.info(`${this._fromMe()} getting ALL balance`)
        return await this.node.cli.getBalanceAll(this.keyInfo.PrivateKey)
    }

    async getBalance(token = TOKEN.PRV) {
        loggerCli.info(`${this._fromMe()} getting  balance of ${token.slice(-8)}`)
        return await this.node.cli.getBalance(this.keyInfo.PrivateKey, token)
    }

    /**
     *
     * @param {IncAccount} receiver
     * @param {number} amount
     * @param {string} token
     * @returns
     */
    async send(receiver, amount, token = TOKEN.PRV) {
        loggerCli.info(`${this.keyInfo.PrivateKey.slice(-8)}` +
            ` send ${amount} of ${token.slice(-8)} to ${receiver._toMe()}`)
    ...
    async addOrder(pairID, nftId, sellToken, sellAmount, minAcceptAmount) {
        loggerCli.info(`${this._fromMe()} (nft: ${nftId.slice(-8)}) Add order book: Sell ${sellAmount} ` +
            `of ${sellToken.slice(-8)} for ${minAcceptAmount} of pair ${pairID}`)
        return await this.node.cli.addOrder(
            this.keyInfo.PrivateKey, pairID, nftId, sellToken, sellAmount, minAcceptAmount)
    }

    async trade(selToken, buyToken, sellAmount, tradingFee, minAcceptAmount, tradingPath, prvFee) {
        loggerCli.info(`${this._fromMe()} swap ${sellAmount} of ${selToken.slice(-8)} for at least ${minAcceptAmount} `
            + `of ${buyToken.slice(-8)}`)
    ...
    async withrawOrder(orderId, pairId, nftId, tokenId) {
        loggerCli.info(`${this._fromMe()} withdraw order book id ${orderId}`)
    ...
    async stake(validator, rewarReceiver) {
        loggerCli.info(`${this._fromMe()} staking me, reward ${rewarReceiver._toMe()}`)
    ...
    async unstake(validator) {
        loggerCli.info(`${this._fromMe()} unstaking ${validator._toMe()}`)
    ...
    async waitBalanceChange({ tokenId = TOKEN.PRV, from = 0, amountChange = 1, checkInterval = 10, timeout = 90 }) {
        loggerCli.info(`${this._fromMe()} wait for balance of ${tokenId.slice(-8)} change at least ${amountChange} ` +
            `from ${from}`)
    ...
    async getMyNtfIds() {
        loggerCli.info(`${this._fromMe()} getting all my NFT IDs`)
...
class RpcAction extends BaseAction {
    async makeRawTx(receiver, amount, fee = 10, privacy = 1) {
        loggerRpc.info(`${this._fromMe()} create raw transfer tx to ${receiver._toMe()}`)
        return await this.node.rpc.createRawTx(this.keyInfo.PrivateKey, receiver.paymentK, amount, fee, privacy)
    }

    async submitKey() {
        loggerRpc.info(`${this._fromMe()} submit key `)
    ...
    async submitKeyEnhanced(fromHeight = 0, reIndex = true) {
        loggerRpc.info(`${this._fromMe()} enhanced submit key `)
    ...
    async getSubmitKeyInfo() {
        loggerRpc.info(`${this._fromMe()} get submit key status`)
...
class SdkAction extends BaseAction {
    ...
    async init({
        ...
        loggerSdk.info("Initialize a new sdk instance");
        await init();
        ...
        loggerSdk.info("Initialize a new sdk wallet");
        ...
        if (this.keyInfo != null) {
            loggerSdk.debug("Set private key");
            await this.sdkWallet.setKey(this.keyInfo.PrivateKey)
        }
        loggerSdk.info("Initialize a new pDEXv3 instance");
    ...
    async getBalance(tokenId = TOKEN.PRV, version = 2) {
        ...
        loggerSdk.info(`${this._fromMe()}: balance of ${tokenId.slice(-8)} = ${bal}`)
        return parseInt(bal)
    }

    async getBalanceAll() {
        loggerSdk.info(`${this._fromMe()} getting ALL balance`)
    ...
    async sendPRV({
        receiver,
        amount,
        fee = ENV.config.minTxFee,
        memo = "",
        version = 2
    }) {
        loggerSdk.info(`${this._fromMe()} sending ${amount} PRV to ${receiver._toMe()}`)
    ...
    async sendTokenToPaymentAddress({
        token,
        address,
        amount,
        fee = 100,
    ...
    async getNftTokens() {
        var nftData = await this.getNftData()
        var nftTokens = []
        for (var datum of nftData) {
            nftTokens.push(datum.nftToken)
        }
        return nftTokens
    }
    ...
    async addOrder({
        ...
        loggerSdk.info(`${this._fromMe}: adding order ${JSON.stringify(order, null, 3)}`)
        let res = await this.pDexV3Instance.createAndSendOrderRequestTx(order);
        return res.txId;
    }
    ...
    async contributeLiquidity({
        ...
        loggerSdk.info(`${this._fromMe()} contribute ${JSON.stringify(arguments[0],null,3)}`)
