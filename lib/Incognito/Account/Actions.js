...
class SdkAction extends BaseAction {
    ...
    async init({ coinService = ENV.urlCoinService, fullNode = ENV.urlFullNode, pubsubService = ENV.urlPubsubService,
        requestService = ENV.urlWebService, apiService = ENV.urlBackend } = {}) {

        console.log("Initialize a new sdk instance");
        await init();
        await wasm.setShardCount('', 8);
        console.log({ coinService : ENV.urlCoinService, fullNode : ENV.urlFullNode, pubsubService : ENV.urlPubsubService,
            requestService : ENV.urlWebService, apiService : ENV.urlBackend });
