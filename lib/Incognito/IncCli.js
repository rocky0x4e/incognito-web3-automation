...
class IncCli extends BaseCli {
    ...
    getBalance(privateKey, token = TOKEN.PRV) {
    ...
    getOutcoin(address, otaKey, tokenID) {
        const output = this.run(`account outcoin --address ${address} --otaKey ${otaKey} --tokenID ${tokenID}`);
        return output
    }
    ...
    myNft(privateK) {
        const output = this.run(`pdeinfo mynft --privateKey ${privateK}`)
        console.log('hoanh output', output);
        return JSON.parse(output.substring(output.indexOf('[')))
    }
}
